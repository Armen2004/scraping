...
class JobsNet extends AbstractWebGsScraper2
{
    private $page = 1;
    private $totalPages = 1;
    private $key = 1;
    private $viewstate;
    private $htxtRowCount;
    ...
    function parseJobList()
    {
        ...
        $this->curl->setOpt(CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:38.0) Gecko/20100101 Firefox/38.0');
        $this->curl->setOpt(CURLOPT_FRESH_CONNECT, false);
        $this->curl->setOpt(CURLOPT_RETURNTRANSFER, true);
        $this->curl->setOpt(CURLOPT_SSL_VERIFYPEER, false);
        $this->curl->setOpt(CURLOPT_FOLLOWLOCATION, true);
        $this->curl->setOpt(CURLOPT_HEADER, true);
        $this->curl->setOpt(CURLOPT_HTTPHEADER, Array("Content-Type: application/x-www-form-urlencoded"));
    ...
    public function loadJobDetails($url)
    {
        ...
        $title = trim($dom->find('span[itemprop=title]', 0)->plaintext);
        $companyname = 'Jobs Net';
        $snippet = trim($dom->find('#description', 0)->plaintext);
        $city = '';
        $state = '';
        $country = '';
//        $location = trim($dom->find('span[id=lblCityValue]', 0)->plaintext);
//        $location = explode('|', $location);
//        if (count($location) > 2) {
//            $city = trim($location[1]);
//            $country = trim($location[2]);
//        }
        $city = trim($dom->find('span[itemprop=addressLocality]', 0)->plaintext);
        $state = trim($dom->find('span[itemprop=addressRegion]', 0)->plaintext);
        $country = 'USA';
        $title = $this->clean($title);
        $snippet = $this->clean($snippet);
        $snippet = preg_replace('/[0-9]+/', '', $snippet);
        $city = $this->clean($city);
        $state = $this->clean($state);
        $country = $this->clean($country);
        $this->setJobToList($url, $this->key, $title, $snippet, $city, $state, $country, date('Y-m-d'), $companyname);
        $this->key++;
    }
    ...
    private function extractjobstourl($content)
    {
        ...
        $urls = array();
        $as = $dom->find('tr[class=resultRow] td a');
        foreach ($as as $a) {
            $url = $a->href;
            if (strlen($url) > 0) {
    ...
    private function getNextPageContent()
    {
        ...
        $this->htxtRowCount = count($dom->find('tr[class=resultRow]'));
        $this->totalPages = count($dom->find('#pagination-container', 0)->find('a[class=number]'));
        return $content;
    }
